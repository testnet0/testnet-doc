...
- **熔断/限流保护**：集成 Resilience4j 2.2.0，工作流与任务执行支持自动熔断与指数退避重试，系统告警通过 WebSocket `/topic/system-alert` 广播
- **配置文件远端同步**：扫描节点支持从服务端拉取配置文件（SHA-256 校验 + symlink 版本管理）
...
- 服务端底层架构进行较大重构，升级 Fastjson 至 v2.0.43
- 底层运行环境升级至 JDK 17（兼容 JDK 8 和 JDK 21）
- Knife4j 升级至 openapi3 标准的 springdoc-openapi
- 内置容器由 tomcat 替换为更轻量、并发更高的 undertow
- 升级 Spring Cloud 至 2021.0.8 级 Spring Cloud Alibaba 2021.0.6.2
- 报表组件 Jimureport 升级至 v1.9.5 最新版
...
- **底层通信升级**：使用 gRPC 高性能 RPC 框架替代 Redis 发布订阅，实现服务端与分布式客户端的极速同步
